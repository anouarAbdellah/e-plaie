...
import { bilans_types } from 'src/app/shared/constants';
...
export class BilansListComponent implements OnInit {
  ...
  constructor(private bilansService: BilansService, private translate: TranslateService) { }
  ...
  toggleStatusChange(data) {
    ...
    this.isLoading = true;
    this.bilansService.status(data.id, {enabled: data.status}).subscribe(
      (result) => {
  ...
  onLoad() {
    ...
    console.log(filters)
    this.bilansService.all({params: filters}).subscribe(
      (result) => {
        console.log(result)
  ...
  exportData() {
    ...
    this.bilansService.all({params: filters, responseType: 'blob' as 'json'}).subscribe(
      (result) => {
        console.log(result);
        const blob = new Blob([result], { type: 'text/csv' });
        saveAs(blob, 'bilans.csv');
        this.isLoading = false;
      }, 
      (error) => {
        console.log(error);
